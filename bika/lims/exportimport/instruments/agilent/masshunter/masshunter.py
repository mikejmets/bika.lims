...
class AgilentMasshunterParser(InstrumentCSVResultsFileParser):
    ...
    def parse_headerline(self, line):
        """ Parses header lines
            ...
            Batch Data Path	D:\MassHunter\GCMS\Terpenes\2017\July\20170711\QuantResults\20170711 Sample Workup
        ...
        #Report Results Data Path	None
            ...
            if splitted[1]:
                self._header[self.HEADERKEY_REPORTRESULTSDATAPATH] = splitted[1]
            else:
                self.warn("File Data Name not found or empty",
                          numline=self._numline, line=line)
    ...
    def parse_quantitationesultsline(self, line):
        ...
        if line.startswith('SampleID'):
            self._end_header = True
            self._quantitationresultsheader = [token.strip() for token in line.split(self.COMMAS) if token.strip()]
            return 0
