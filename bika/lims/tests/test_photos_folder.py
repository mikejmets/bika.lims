...
"Sample 2", HHS14001,          3/9/2014,    3/9/2014,,Toilet,     Liquids,     Water,     Cup,          0,              Normal,  1,                                   0,             0,0,0,0,0,1
    ...
    def test_ar_photos(self):
        path = os.path.dirname(__file__)
        directory = '%s/files/photos' % path
        folder = self.portal.bika_setup.setPhotosFolder(directory)
        folder = self.portal.bika_setup.getPhotosFolder()
        transaction.commit()
        if not os.path.isdir(folder):
            self.fail("Folder  %s not found" % folder)
        browser = self.getBrowser()
        portal_url = self.portal.absolute_url()
        url = '{}/folderphotos'.format(portal_url)
        browser.open(url)
        transaction.commit()
        time.sleep(3)
        errorsfolder = '{}/errors'.format(directory)
        for photo in os.listdir(errorsfolder):
            source = '{}/{}'.format(errorsfolder, photo)
            destination = '{}/{}'.format(directory, photo)
            os.rename(source, destination)

        image1 = '{}/archives/1-0001-R01'.format(directory)
        destination = '{}/1-0001-R01'.format(directory)
        os.rename(image1, destination)
        image2 = '{}/archives/1-0002-R01'.format(directory)
        destination = '{}/1-0002-R01'.format(directory)
        os.rename(image2, destination)
        ...
        for ar in ars:
            ...
            if not obj.getAttachment():
                self.fail("Attachment on an AR %s not found" % ar)
