...
class TestInstrumentImport(BikaSimpleTestCase):
    ...
    def test_Shimadzu_ICPE9000Import_AutoTransition_SamplingEnabled(self):
...
"Sample 2", HHS14001,          3/9/2014,    3/9/2014,,Toilet,     Liquids,     Water,     Cup,          0,              Normal,  1,                                   0,             0,0,0,0,0,1
    ...
    def test_Shimadzu_ICPE9000Import_NoAutoTransition_SamplingDisabled(self):
...
"Sample 2", HHS14001,          3/9/2014,    3/9/2014,,Toilet,     Liquids,     Water,     Cup,          0,              Normal,  1,                                   0,             0,0,0,0,0,1
        ...
        for ar in ars:
            if ar.id == '1-0001-R01':
                ...
                state = workflow.getInfoFor(analysis, 'review_state')
                if state == 'to_be_verified':
                    self.fail('Auto Transition occured for:{}'.format(analysis))
                if analysis.getResult() != '20.0':
                    self.fail("AR: %s  Result did not get updated" % ar.id)
                state = workflow.getInfoFor(analysis, 'review_state')
